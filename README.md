...
The Music tab loads `https://bcsdny.net/~a/`, the actual music iframe discovered inside the user-supplied learning/health page, in an existing Scramjet/libcurl proxy frame. Opening the inner player directly avoids the outer wrapper and its corrupted rendering through the proxy. Its original search, song list, audio playback and controls remain in the embedded source. The previous SoundCloud catalog is retained as a file but is no longer the active music interface.
...
The Roblox-only launcher opens the official now.gg Roblox URL through the existing Scramjet/libcurl controller. Neon branding applies to the surrounding UI; now.gg runs the remote service. Reload, fullscreen, close, and a direct provider link are included. During local verification, the provider redirected its Roblox URL to the general homepage; the launcher detects that homepage and tries the user-supplied alternate Roblox URL, `https://nowgg.lol/apps/a/19900/b.html`. The Roblox tile launches the player directly; the extra description and launch buttons have been removed. Its Roblox loading screen rendered through the proxy during local testing, but a playable session was not confirmed. Roblox streaming has not been verified; provider queues, account requirements, and availability are outside Neon’s control. No other cloud games, provider accounts, subscriptions, or gaming servers were created.
