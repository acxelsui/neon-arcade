...
The Roblox-only launcher opens the official now.gg Roblox URL through the existing Scramjet/libcurl controller. Neon branding applies to the surrounding UI; now.gg runs the remote service. Reload, fullscreen, close, and a direct provider link are included. During local verification, the provider redirected its Roblox URL to the general homepage; the launcher detects that homepage and tries the user-supplied alternate Roblox URL, `https://nowgg.lol/apps/a/19900/b.html`. The alternate is also available as a separate button. Its Roblox loading screen rendered through the proxy during local testing, but a playable session was not confirmed. Roblox streaming has not been verified; provider queues, account requirements, and availability are outside Neon’s control. No other cloud games, provider accounts, subscriptions, or gaming servers were created.
