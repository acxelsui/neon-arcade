...
function makeGameFrame(game,doc=document){const frame=doc.createElement('iframe');frame.src=new URL("/game-runner.html?id="+encodeURIComponent(game.id),location.origin).href;frame.title=game.name;frame.allow='cross-origin-isolated; autoplay; fullscreen; gamepad';frame.allowFullscreen=true;frame.setAttribute('sandbox','allow-scripts allow-same-origin allow-forms allow-pointer-lock allow-downloads allow-modals');return frame}
...
function closeGame(){if(document.fullscreenElement===$('#player'))document.exitFullscreen().catch(()=>{});window.dispatchEvent(new CustomEvent('neon-game',{detail:null}));$('#game-frame-wrap').replaceChildren();$('#player').hidden=true;$('header').inert=false;$('main').inert=false;document.body.style.overflow='';showPage('games');previousFocus?.focus()}
$('#close-game').onclick=closeGame;
$('#game-fullscreen').onclick=async()=>{try{if(document.fullscreenElement===$('#player'))await document.exitFullscreen();else if($('#player').requestFullscreen)await $('#player').requestFullscreen();else toast('Fullscreen is unavailable in this browser.')}catch{toast('Fullscreen is unavailable in this browser.')}};
document.addEventListener('fullscreenchange',()=>{const active=document.fullscreenElement===$('#player'),button=$('#game-fullscreen');button.textContent=active?'⛶ Exit fullscreen':'⛶ Fullscreen';button.setAttribute('aria-label',active?'Exit game fullscreen':'Enter game fullscreen');button.setAttribute('aria-pressed',String(active))});
