...
document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!$('#player').hidden)closeGame();if(e.key==='/'&&$('#games').hidden===false&&!['INPUT','TEXTAREA'].includes(document.activeElement.tagName)){e.preventDefault();$('#game-filter').focus()}if(e.key==='Tab'&&!$('#player').hidden){const first=$('#retry-game'),last=$('#close-game');if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}}});
